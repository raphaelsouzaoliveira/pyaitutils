...
def get_metrics(y_test, y_pred, class_names = None, save_path=None):
    ...
    if(len(y_test.shape) == 1):
        ...
        #y_pred = multiclass_predict_nd_to_1d(y_pred)
        #y_true = multiclass_predict_nd_to_1d(y_true)
        matrix = metrics.confusion_matrix(multiclass_predict_nd_to_1d(y_test), multiclass_predict_nd_to_1d(y_pred))
...
def plot_confusion_matrix(y_test, y_pred, class_names=None, save_path=None, visualize=False, cmap=None, normalize=True, labels=True, title='Matriz de confusão'):
    ...
    #rotulos = ['True Positive','False Negative','False Positive','True Negative','False Positive','True Negative','True Negative','False Positive','True Negative']
    ...
    if normalize:
        ...
        perc_cm = perc_cm*100 #modificação wenisten para poder elevar para percetual o resultado.

        
    fig = plt.figure(figsize=(15, 13)) # (8, 6))
    ...
    if class_names is not None:
        ...
        plt.xticks(tick_marks, class_names, fontsize=16, rotation = 45, ha='right', rotation_mode="anchor")
    ...
    if labels:
        for i, j in itertools.product(range(cm.shape[0]), range(cm.shape[1])):
            if normalize:
                ...
                """
                plt.text(j, i, rotulos[contador]+'\n'+'{:0.2f}%'.format(cm[i, j]) , fontsize=16,
    ...
    if(save_path is not None):
        if(not os.path.isdir(save_path)):
            os.makedirs(save_path, exist_ok=True)
        fig.savefig(os.path.join(save_path, 'confusion_matriz.png'), dpi=180, bbox_inches='tight')
    if(visualize):
        plt.show()
    plt.close()
...
def plot_auc_roc_multi_class(y_test, y_pred, class_names, save_path=None):
        ...
        plt.plot(fpr["micro"], tpr["micro"],
                 ...
                 color='deeppink', linestyle=':', linewidth=4, alpha=1)

        plt.plot(fpr["macro"], tpr["macro"],
                 label='macro-average ROC curve (area = {0:0.2f})'
                       ''.format(roc_auc["macro"]),
                 color='navy', linestyle=':', linewidth=4, alpha=1)
        ...
        for i, color in zip(range(n_classes), colors):
            ...
            plt.plot(fpr[i], tpr[i], color=color, lw=lw,
                     ...
                     ''.format(class_names[i], roc_auc[i]), alpha=1)

        plt.plot([0, 1], [0, 1], 'k--', lw=lw, alpha=1)
        plt.xlim([0.0, 1.0])
        plt.ylim([0.0, 1.05])
...
def plot_prc_auc_multiclass(y_test, y_pred, class_names, save_path=None):   
        ...
        for i in range(n_classes):
            precision[i], recall[i], _ = metrics.precision_recall_curve(y_test[:, i],
                                                                y_pred[:, i])
            average_precision[i] = metrics.average_precision_score(y_test[:, i], y_pred[:, i])
        ...
        for f_score in f_scores:
            ...
            l, = plt.plot(x[y >= 0], y[y >= 0], color='gray', alpha=0.2)# alpha=0.2)
            plt.annotate('f1={0:0.1f}'.format(f_score), xy=(0.9, y[45] + 0.02))

        lines.append(l)
        labels.append('iso-f1 curves')
        l, = plt.plot(recall["micro"], precision["micro"], color='gold', lw=2, alpha=1)
        lines.append(l)
        labels.append('micro-average Precision-recall (area = {0:0.2f})'
                      ''.format(average_precision["micro"]))

        for i, color in zip(range(n_classes), colors):
            l, = plt.plot(recall[i], precision[i], color=color, lw=2, alpha=1)
            lines.append(l)
